    ...
    L.Map = L.Map.extend({
        ...
        _initSync: function () {
            ...
            originalMap.on('zoomend', function () {
                var self = this;
                originalMap._syncMaps.forEach(function (toSync) {
                    toSync.setZoom(originalMap.getZoom(), {
                        animate: false,
                        reset: false
                    });
                });
            ...
            originalMap.dragging._draggable._updatePosition = function () {
                var orig = originalMap.dragging._draggable._element._leaflet_pos;

                L.Draggable.prototype._updatePosition.call(this);

                // TODO this fails when the map is resized
                var updated = originalMap.dragging._draggable._element._leaflet_pos,
                    dx = updated.x - orig.x,
                    dy = updated.y - orig.y;

                originalMap._syncMaps.forEach(function (toSync) {
                    var current = toSync.dragging._draggable._element._leaflet_pos,
                        newPos = L.point(current.x + dx, current.y + dy);
