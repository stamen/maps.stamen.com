...
po.queue = (function() {
  ...
  function process() {
    ...
    if (active === 0 && queued.length === 0) {
      po.queue.dispatch({type: "complete", active: active, queued: queued.length});
    } else {
      po.queue.dispatch({type: "update", active: active, queued: queued.length});
    }
  }
  ...
  var q = {text: text, xml: xml, json: json, image: image};
  q.dispatch = po.dispatch(q);
  return q;
...
po.layer = function(load, unload) {
  var layer = {},
      cache = layer.cache = po.cache(load, unload).size(128),
      tile = true,
      visible = true,
      zoom,
      id,
  ...
  function move() {
    ...
    for (var key in newLocks) {
      var t = newLocks[key],
          ...
          translate = [(t.x = tileSize.x * (t.column - tileCenter.column * k)),
                       (t.y = tileSize.y * (t.row - tileCenter.row * k))];
      t.element.setAttribute("transform", "translate(" + translate + ")");
      t.translate = translate; // XXX SA: save translation on tile object
    }
